...
pkgver=13.0.r23422.7033319
pkgrel=1
pkgdesc='XBMC Media Center Retroplayer'
provides=('xbmc')
...
pkgver() {
  ...
  printf "13.0.r%s.%s" "$(git rev-list --count HEAD)" "$(git rev-parse --short HEAD)"
}
...
#   --enable-external-ffmpeg      enable use of external ffmpeg libraries (default is no) 'Linux only'
  ...
  ./configure --prefix='/usr' --exec-prefix='/usr' --enable-{pulse,ffmpeg-libvorbis,libbluray,external-libraries} --disable-external-ffmpeg
  make
}
